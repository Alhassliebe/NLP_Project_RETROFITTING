...
Extracts the top-15 pairs from WN_all with the largest INCREASE in cosine similarity
after retrofitting (expected: true synonyms moving closer), and the top-15 with the
largest DECREASE (expected: pairs that were closer than they should have been, pulled
apart by lexicon structure). Analogous to Faruqui et al. (2015) Table 6.
"""
import sys, os
import pandas as pd
import numpy as np
...
# Step 4: compute Δ cos sim for each pair — vectorized for speed
def cos(v1, v2): return v1 @ v2 / (np.linalg.norm(v1) * np.linalg.norm(v2))
...
# Step 5: top-K most improved and most worsened
...
# Step 6: save full table + top-K subsets
