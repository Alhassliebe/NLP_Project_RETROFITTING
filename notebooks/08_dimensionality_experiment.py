...
print(f"Dimensionality experiment over dims={DIMS}", flush=True)
all_rows = []
for d in DIMS:
    ...
    if result.returncode != 0:
        print(f"!! Subprocess for dim={d} failed with code {result.returncode}", flush=True)
        print(f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}", flush=True)
        sys.exit(1)
    # Parse JSON result after the marker
    out_lines = result.stdout.splitlines()
    print("\n".join(out_lines[:-2]), flush=True)  # child progress prints
...
print("\nSaved to results/dimensionality_experiment.csv", flush=True)
...
print("Saved figures/dimensionality.png", flush=True)
...
print("Saved figures/dimensionality_delta.png", flush=True)
